...
BARS_ENABLED = os.getenv("INTRADAY_BARS_ENABLED", "false").lower() == "true"
...
def _flush_bars(now: float) -> None:
    """확정 1분봉 회수 → JSONL append (INTRADAY_BARS_ENABLED 시만·실패 격리)."""
    if _BAR_AGG is None:
        return
    try:
        done = _BAR_AGG.roll(now)
        if done:
            _ib.append_bars(done)
    except Exception as e:
...
def _apply(latest: dict, rec: dict, *, delayed: bool) -> None:
    ...
    if rec["kind"] == "trade":
        ...
        if rec.get("volume") is not None:
            e["volume"] = rec["volume"]
        if _BAR_AGG is not None and rec.get("price"):
            try:
                _BAR_AGG.on_tick(sym, rec["price"], rec.get("volume"), e["ts"],
                                 _ib.market_of(sym))
            except Exception:
                pass   # bar 집계 실패가 시세 반영을 깨지 않게
    else:
