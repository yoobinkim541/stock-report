...
## 2026-06-05 — Hermes server-healthcheck가 스톡봇 중복 재시작 알림 발송

### 증상

Hermes `server-healthcheck` cron이 30분마다 아래 경고를 보냈다.

```text
🤖 텔레그램 봇 재시작됨 (이전 프로세스 없음)
```

동시에 `/tmp/barbell_bot.log`에는 이미 실행 중인 봇이 lock을 점유하고 있어 새 프로세스가 종료된 흔적이 남았다.

### 원인

stock-report의 canonical PID/lock 경로는 아래로 변경됐다.

```text
/home/ubuntu/.local/state/stock-report/barbell_bot.pid
/home/ubuntu/.local/state/stock-report/barbell_bot.lock
```

하지만 Hermes cron script `~/.hermes/scripts/healthcheck.sh`는 여전히 `/tmp/barbell_bot.pid`만 확인하고, 없으면 `python3 telegram_bot.py`를 직접 실행했다. 그래서 실제 봇은 살아 있는데도 PID 파일 없음으로 오판했다.

### 수정 내용

- `~/.hermes/scripts/healthcheck.sh`의 PID 파일 경로를 canonical 경로로 변경
- PID 파일이 없을 때도 `pgrep -f "python.*telegram_bot.py"`로 실제 실행 여부를 한 번 더 확인
- 직접 `python3 telegram_bot.py` 실행 금지
- 재시작이 필요하면 `/home/ubuntu/projects/stock-report/bot_watchdog.sh`에 위임

### 재발 방지

- 스톡봇 시작/재시작 로직은 `telegram_bot.py`, `bot_watchdog.sh`, `bot_healthcheck.py`, Hermes `server-healthcheck`가 같은 PID/lock 경로를 써야 한다.
- 외부 cron/Hermes script에서 `telegram_bot.py`를 직접 `python3`로 띄우지 말고 프로젝트 watchdog에 위임한다.
- 관련 변경 후 `~/.hermes/scripts/healthcheck.sh`를 직접 실행하고, Hermes cron 산출물에서도 `✅ 모두 정상`을 확인한다.
