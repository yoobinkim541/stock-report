...
- AI 위키: shared-memory에 기록된 대화/메모를 승격해 다시 읽는 지식 카드 레이어다. 위키 페이지는 `surface=wiki` 기록으로 저장되고 다음 답변의 컨텍스트에 다시 들어간다.
- Auto-curation: 답변이 끝난 뒤 재사용 가능한 규칙/결정/주의점은 LLM 우선, 휴리스틱 폴백으로 위키 카드로 자동 정리한다.
...
- `AGENT_CONSOLE_WIKI_AUTOCURATE_ENABLED`: `0`이면 답변 후 위키 자동 정리를 비활성화한다. 기본값은 `1`이다.
