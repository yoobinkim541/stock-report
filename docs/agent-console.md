...
- AI 위키: 대화와 메모를 승격해 `shared-memory`에 `surface=wiki` 기록으로 저장하고, 다음 답변에서 다시 읽는 지식층이다. 뉴스/메모는 가능한 한 원문(body_raw)을 함께 보존하고, 자동 승격은 재사용 가능한 규칙·결정·편향 교정만 좁게 올린다.
...
대시보드 사이드바에서 `AI 콘솔`과 `AI 위키`를 연다.
...
이 콘솔은 기존 대시보드를 대체하지 않는다. 대시보드는 차트와 운영 화면, 에이전트 콘솔은 맥락 축적과 질문 응답, 전략 실험의 가벼운 작업대 역할로 나눈다. `AI 위키`는 그 사이에서 반복되는 판단을 정리해 다음 답변에 재사용하는 지식 카드 레이어다.