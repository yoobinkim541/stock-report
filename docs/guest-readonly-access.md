...
현재 게스트는 시장 정보만 본다. "자기 포트폴리오 평가"까지 주려면 `user_id` 스코프
저장소(이미 구축)에 게스트별 보유 종목을 입력받아 **sanitized 포트폴리오 평가**
(평가액·손익·수익률 = 그들 데이터, 처방 없음)를 추가하면 된다. 이때도 매매신호·
리밸런싱 지시는 제외해 법적 경계를 유지한다.
