...
정규 진입: score ≥ θ_entry(0.55) → risk_per_trade 100%
탐색 진입: θ_explore(KR 0.40 / US 0.48) ≤ score < θ_entry
         → risk_per_trade × explore_risk_mult(KR 35% / US 50%)
         AND 하드가드 전부 통과 (극단 스프레드·신선도·일손실예산·세션마감버퍼)
```
...
```
주수 = floor( min(슬리브NAV × risk_per_trade × entry_risk_mult, 남은 일손실예산) / (손절폭+마찰) )
...
| `INTRADAY_EXPLORE_ENABLED` | `true` | 정규 진입선 아래 탐색 진입 허용. shadow 표본 축적용 |
| `INTRADAY_EXPLORE_ENTRY_KR` / `INTRADAY_EXPLORE_RISK_MULT_KR` | `0.40` / `0.35` | KR 탐색 진입. 하루 1건 수준의 콜드스타트를 피하고 손실예산 안에서 표본을 늘림 |
| `INTRADAY_EXPLORE_ENTRY_US` / `INTRADAY_EXPLORE_RISK_MULT_US` | `0.48` / `0.50` | US 탐색 진입. 글로벌 `INTRADAY_EXPLORE_ENTRY` / `INTRADAY_EXPLORE_RISK_MULT`로 일괄 override 가능 |
| `INTRADAY_LEVERAGE_ENABLED` | `true` | US 단기에서 기초자산 신호를 레버리지 ETF 체결로 매핑 |
| `INTRADAY_LEVERAGE_MAP` | `QQQ:TQQQ,NVDA:NVDL,...` | 기초자산:체결ETF 매핑. watchlist 는 양쪽 모두 구독 |
| `INTRADAY_MAX_TRADES_DAY` | `0` | `0`이면 거래횟수 무제한. 손실예산이 실제 제한자 |
| `INTRADAY_MAX_CONCURRENT_POS` | `0` | `0`이면 동시 포지션 수 제한 없음. 시장별 `_KR`/`_US` override 가능 |
| `INTRADAY_COOLDOWN_MIN` | `0` | `0`이면 같은 종목 즉시 재진입 허용. 필요 시에만 손실 후 쿨다운을 선택적으로 켬 |
| `INTRADAY_DAILY_LOSS_HALT` | `0.015` | 슬리브 −1.5% 도달 시 당일 신규진입 정지·전량 청산. 신규 진입 수량도 남은 손실예산 안으로 사전 축소 |
| `INTRADAY_MAX_SPREAD_BPS_KR` / `INTRADAY_MAX_SPREAD_BPS_US` | `25` / `5` | 소프트 스프레드 기준. 실제 마찰·수량 산식에는 관측 스프레드가 반영됨 |
