...
def _reward_risk(s: EntryScore) -> tuple[float, float, float, bool]:
    ...
    risk = max(raw_risk, DISPLAY_RISK_FLOOR)
    adjusted_rr = reward / risk if risk > 0 else 0.0
    inflated = raw_risk > 0 and raw_risk < DISPLAY_RISK_FLOOR and raw_rr >= max(adjusted_rr * 2, 5.0)
...
def _level_pct_notes(s: EntryScore) -> tuple[str, str]:
    """목표/손절 산식 설명. 손절은 P25와 최소 -3% 룰이 함께 적용됨."""
    target_pct = max(s.upside_p75_20d, 0.02)
    stop_pct = min(s.downside_p25_20d, -0.03)
...
def format_alert_message(s: EntryScore) -> str:
    ...
    rr_str = _rr_display(s)
    reasons = _display_reasons(s)
    rr_basis = f"기대수익 {_fmt_pct(s.expected_ret_20d)} / 위험 {_fmt_pct(-risk_pct)}"
    caution_lines = [
        f"  표본 {s.n_similar}건({_sample_quality(s.n_similar)}) — 적을수록 승률·분위수 흔들림 큼",
        f"  60일 승률 {s.win_prob_60d*100:.0f}% · 기대 {_fmt_pct(s.expected_ret_60d)} — 20일보다 약하면 짧게 관리",
    ]
    if inflated_rr:
        caution_lines.append(f"  원시 손익비 {raw_rr:.1f}×는 하방 P25 {_fmt_pct(s.downside_p25_20d)}가 너무 작아 과대 표시 가능")
    caution_lines.append("  아래 참고 지표가 매도/피벗 하회면 통계 신호와 기술 추세가 충돌")

    lines = [
        f"🔔 진입 후보(정보형) — {short_name} ({s.ticker})",
        f"{emoji} {full_name} · 점수 {s.score:.2f}/1.00",
        f"",
        f"[ 한줄 판단 ]",
        ...
        f"  2. 유사 과거 {s.n_similar}건: 20d 승률 {s.win_prob_20d*100:.0f}%, 기대수익 {_fmt_pct(s.expected_ret_20d)}",
        ...
        *caution_lines,
        f"",
        f"[ 현재 상태 ]",
        f"  20d 모멘텀 {_fmt_pct(s.current_mom_20d)} · 60d {_fmt_pct(s.current_mom_60d)} · VIX {s.current_vix:.1f}",
        f"",
        f"[ 유사 과거 분석 ]",
        f"  표본:     {s.n_similar}건 ({_sample_quality(s.n_similar)})",
