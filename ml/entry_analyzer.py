...
def _entry_summary(s: EntryScore, rr: float) -> str:
    win = s.win_prob_20d
...
def format_alert_message(s: EntryScore) -> str:
    ...
    lines = [
        f"🔔 진입 후보 감지(정보형) — {s.ticker} · {short_name}",
        f"{emoji} {full_name} · 점수 {s.score:.2f}/1.00",
        f"",
        f"[ 한줄 판단 ]",
        f"  {_entry_summary(s, rr)}",
        f"",
        f"[ 왜 떴나 ]",
        ...
        f"  3. 손익비 {rr_str}: 목표 {_fmt_pct(target_pct)} vs 손절 {_fmt_pct(stop_pct)}",
        f"",
        f"[ 현재 상태 ]",
        ...
        f"  현재가:     {_price_str(s)}",
        f"  관찰/분할:  {buy_str}",
        f"  목표 참고:  {target_str} ({target_note})",
        f"  무효화선:   {stop_str} ({stop_note})",
        f"",
        f"[ 리스크 체크 ]",
        f"  60일 승률 {s.win_prob_60d*100:.0f}% · 60일 기대 {_fmt_pct(s.expected_ret_60d)} — 장기 우위 약하면 짧게 관리",
        f"  분포 기반 레벨입니다. 무효화선 이탈 시 신호 무효로 취급",
        f"",
        f"💡 {' · '.join(s.reasons[:3])}",
        f"",
