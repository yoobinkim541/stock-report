...
| `lib/agent_memory.py` | 공유 에이전트 메모리 — events.jsonl(append-only)·사용자 노트북(타임스탬프→일별 롤업 압축 상태머신)·`memory_summary.md` 2계층 패킷(사용자 기억+외부[최신 리포트 요약·뉴스 다이제스트]) → advisor 프롬프트 주입(참고·지시 아님)+`/ask` 대화 축적. 비밀값 레닥션·15분 외부 캐시. 원저작 FinanceAgentGUI(devninjadev·BSD-3) 포팅. **주간 프로필 증류**(`--distill` 크론 토 01:10 — 롤업+행동신호[실거래·알림]를 LLM 체인으로 ≤900자 프로필화·실패 시 기존 유지) | `~/.local/share/stock-report/shared-memory/` |
| `lib/world_memory.py` | **월드 메모리** — 시장 이슈 영구 축적(SQLite·dedupe 멱등) + FTS5 타임라인 검색 + 스토리 상태 체인(supersede — '어디서 시작해 여기까지' 서술 재료). 수집원: 뉴스 LLM 라벨·중요 속보(자동 훅). 소비: /ask 회고 주입·메모리 패킷 외부계층·대시보드 🧭 축적 맥락. FinanceAgentGUI 스키마 이식(BSD-3·임베딩→FTS5 대체)·컨텍스트 전용(판단 반영은 news 축 게이트만) | `~/.local/share/stock-report/shared-memory/world_issue_log.sqlite3` |
...
| `crons/kiwoom_sync_rest.py` | 키움 REST API 국내주식 잔고 동기화 + **해외주식(미국) 잔고**(ust21070 read-only — 기본 diff 보고만·`OVERSEAS_SYNC_SOURCE=kiwoom` 시 overseas_general 반영) + **해외 체결내역**(ust21100 최근 7일 → `trade_events` 원장[event_id=거래번호 멱등] → 대시보드 가격차트 ▲▼ 마커. **주식모으기**: 전용 TR 부재 — 모으기 체결이 일반 매수로 기록되며 거래종류명 note 보존[약정 자체는 REST 미제공·정직 한계]) | 평일 23:35 UTC |
...
>
> **TV급 도구·쿠키 세션·LLM 연관(T-series):** ① **드로잉 도구 확장** — 기존 4종에 │수직선·✚크로스라인·↗레이·⤢연장선(기울기 외삽)·📝메모(window.prompt)·📈롱/📉숏 포지션(진입→목표 드래그=보상존+RR 1:1 손절존 자동·rect 편집 가능·%·라벨) 추가. 풀뷰(chart_full)는 `dock=True` 로 **좌측 세로 도구 독**(TradingView 배치·`#wrap.dock` CSS). ② **쿠키 세션**(auth.py) — 카드 앵커(?tk=)·F5·재기동 리로드=새 세션이라 비번 재요구되던 것 → 로그인 시 HMAC 서명 토큰 쿠키(30일·`issue_token/verify_token` 순수) 발급, `st.context.cookies` 자동 재인증. 키=sha256(비번+`~/.cache/dashboard_auth_salt`) — 비번 변경/salt 삭제=전 기기 로그아웃. 워치독 리로드도 무입력 복귀 + **health fetch 2.5s 타임아웃·연속 3회 실패부터 down**(단발 리셋 오탐 리로드 제거). ③ **가드 계층화**(plotly_embed) — guard boolean→**카운터**(겹친 relayout 의 .then 조기해제 창 제거) + 도형 이벤트 전용 `drawGuard` 분리(applyDraw/복원 메아리 1차 차단·내용 기반 방어는 2차·팬 애니 중 도형 이벤트 삼킴 해결). ④ **🤖 LLM 연관 종목**(`providers/llm_related.py`) — hermes chat(codex) 로 현 종목 연관/경쟁 3~5개+한줄 근거. **환각 방어 = normalize_input 화이트리스트**(미해석 티커 폐기)·relation enum·24h 디스크 캐시(`~/reports/ml-cache/llm_related/`)·버튼 게이트·정직 라벨(검증 안 된 참고용). env: `DASH_LLM_RELATED_ENABLED`(기본 1)·`_MODEL`·`_PROVIDER`·`_TIMEOUT`. ⑤ **🤖 AI 종목 분석**(`providers/llm_analysis.py`) — 종목분석 페이지 expander: **이미 계산된 지표만** DATA 블록으로 조립(`_llm_analysis_facts` — 기술 프로필·밸류·컨센서스·분기 펀더멘털·기관매집·연간추세, 숫자 반올림, 추가 네트워크 0) → 구조화 해설(요약·강점/리스크·밸류 맥락·기술 위치·체크포인트). **방어 3중**: DATA 밖 수치 금지 지시 + 금지어 필터(매수하/목표가/확실… — 항목 폐기·요약이면 전체 폐기) + **강점/리스크 균형 강제**(한쪽 비면 폐기). 24h 디스크 캐시(`~/reports/ml-cache/llm_analysis/`)·버튼 게이트·표시 전용(판단 미반영). env: `DASH_LLM_ANALYSIS_ENABLED`(기본 1)·`_MODEL`·`_PROVIDER`·`_TIMEOUT`(90s) — 모델 폴백은 `INVESTMENT_REPORT_LLM_*` (**.env=gpt-5.5 — mini 는 402/400**). ⚠️ AppTest 함정: 버튼 클릭=풀 rerun 이라 `st.rerun(scope="fragment")` 가 예외 — 클릭 결과는 **같은 런 폴스루 렌더** + 예외 폴백 rerun 패턴.
>
> **TV 갭 반영 2차(V-series — 유저 TV 스크린샷 대비 미구현분):** ① **드로잉 3종** — 📐 **회귀추세**(구간 박스 드래그=종가 OLS 중심선+±2σ 채널·변화율 라벨·로그축은 log 공간 회귀), ⚓ **고정 VWAP**(짧게 긋기=앵커 봉부터 (저+고+종)/3×거래량 누적 — 도형이 아닌 **트레이스**[hover 지원], 영속화는 `tndraw` 문서에 **앵커 ms 목록(vwaps)만** 저장→로드 시 bounds 재계산·지우기가 `clearVwaps`로 트레이스 정리), 📊 **고정범위 볼륨 프로필**(구간 박스=24 가격빈 히스토그램[고저 걸친 빈 균등 분배]+POC 라인). 비교(%) 모드는 VWAP·프로필 차단(4열 bounds 에 실가격·OHLC 없음)·파생 도형(tool-reg/vprof)은 자석 스냅 제외. ② **지표 6종** — 상단: 켈트너 채널(EMA20±2×ATR10)·KAMA(10·2·30·`kama_series` 순수)·샹들리에 엑시트(22·3×ATR 롱/숏 스탑) / 하단 서브패널: Aroon(25)·볼린저 %b·PVT — **패널 동적 배정 5→8행**(6+ 는 가격 0.40+서브 균등·높이 900+85/패널). ③ **펀더멘털 서브패널(W2 — TV img08)**: 하단 지표 '펀더멘털' = 분기 매출 바(**폭=간격 45% 명시** — 자동폭은 분기 전체를 채워 뭉툭)+순이익 라인(적자 분기 빨강 마커)+hover 순마진·`providers/earnings_data.quarterly_fundamentals`(yfinance 분기/연간 손익 → {date,revenue,net_income,margin}·12h 캐시·**실패는 미캐시**=재시도 허용·`_fund_rows` 순수 파서)·분기<4개면 연간 폴백·ETF/매크로는 손익 없음→패널 생략+정직 캡션(`charts.fmt_big` T/B/M 축약). ④ **TV 갭 3차(X-series — 백로그 소진)**: ∥ 평행 채널(①기준선 ②폭 — 2단계 `pending` 상태기계)·🍴 앤드류스 피치포크(①P0→P1 ②P2 — 중앙선+양 갈래·프리뷰 `-pre` 이름은 완성 시 제거)·🪜 갠 팬(그은 선=1×1·배율 9레이)·𝐄 피보 확장(①A→B ②C — 축공간 파동×0.382~2 레벨)·⏱ 피보 타임존(앵커+피보 봉수 수직선)·🌀 순환선(그은 폭=주기 반복·상한 40)·🔤 엘리엇/◇ XABCD(점마다 짧게 긋기=폴리라인+자동 라벨·버튼 재클릭/만점 완료·프리뷰 교체식=패턴 1개)·🧱 세션별 볼륨 프로파일(인트라데이 전용 가드·최근 10세션×16빈·vprof 분배 로직 재사용). 남은 것: 차트 내 전략 백테스트(무엣지 원칙상 리서치 탭 유지)·lightweight-charts 하이브리드(아키텍처 논의 필요 — 드로잉 스택 전체 재구현 비용).
>
...
## 문서 유지 규칙 (README·CLAUDE.md 동기화)

**작업으로 아래 중 하나라도 바뀌면, 같은 작업(커밋/PR) 안에서 문서를 함께 갱신한다 — 문서 갱신은 별도 후속작업이 아니라 그 변경의 일부다.**

| 무엇이 바뀌면 | 갱신할 문서 |
|---------------|-------------|
| 사용자에게 보이는 동작·명령어·UX·기능 | `README.md` — **`## 🆕 최근 업그레이드` 최상단에 새 항목**(날짜·한 줄 요약·바뀐 파일) |
| 파일 역할·아키텍처·상태파일·데이터 흐름 | `CLAUDE.md` — 해당 표/섹션(파일 역할·아키텍처 핵심·출력 파일) |
| 새 env var / 기본값·의미 변경 | `CLAUDE.md` **환경변수** 표 (+ 사용자 노출 스위치는 README 도) |
| 크론·스크립트·운영 절차(워치독·배포·구동) | `CLAUDE.md` 해당 섹션 + `deploy/crontab.stock-report` 단일 진실원 |
| 안전 불변식(주문 0·read-only·락·fail-closed) | `CLAUDE.md` **안전 규칙** |

**원칙**
- **실제 반영된 것만 기록** — 계획·미완·날조 금지. 문서는 코드의 현재 상태와 일치해야 한다(과거형 약속 아님).
- **순수 내부 리팩터**(사용자 관점·파일 역할·env·운영 절차 무변)면 README `최근 업그레이드`는 생략 가능하나, 파일 역할이 바뀌었으면 `CLAUDE.md` 표는 갱신한다.
- 커밋/PR **전에** 갱신한다. 코드 diff 를 낼 때 문서 diff 가 없다면, 위 표에 걸리는 변경이 정말 없는지 자문한 뒤 진행.
- 새 항목은 기존 스타일(굵은 리드인 + 간결한 설명·바뀐 파일 경로 병기)을 따른다.
