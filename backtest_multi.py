...
# -*- coding: utf-8 -*-
"""
backtest_multi.py — 글로벌 포트폴리오 전략 종합 비교 (5년 / 10년 / 20년)
...
Usage:
  python3 backtest_multi.py                     # 5 / 10 / 20년 모두
...
_TODAY = datetime.now()
PERIODS = {
    f"20년 ({_TODAY.year-20}~{_TODAY.year})": f"{_TODAY.year-20}-01-01",
    f"10년 ({_TODAY.year-10}~{_TODAY.year})": f"{_TODAY.year-10}-01-01",
}
...
ALL_TICKERS = ["QQQ", "SPY", "EFA", "EEM", "TLT", "IEF", "SHY", "BIL",
...
def download_all(start: str) -> pd.DataFrame:
    ...
    for col, base, mult, drag in [
        ...
        if col not in close.columns:
            ...
            mask = close[col].isna()
            close.loc[mask, col] = synth.loc[mask]
...
def build_report(period_logs: dict) -> str:
    ...
    longest_years = max(period_logs.keys())
    _, longest_logs = period_logs[longest_years]
    ...
    if ib_log:
        ...
        lines += [
            "", f"━━━ {longest_years}년 포트폴리오 추이 (④IB v2.2 VIX ●) ━━━━━━━━━━━",
...
def main():
    ...
    parser.add_argument("--period", type=int, choices=[5, 10, 20],
                        help="단일 기간만 실행 (5/10/20년)")
    ...
    if args.start:
        ...
        key = next((k for k in PERIODS if str(args.period) in k), None)
