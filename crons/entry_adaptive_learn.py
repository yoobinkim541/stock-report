...
def _samples() -> list[tuple[float, float]]:
    """실전 outcome → [(score, r_multiple)], **실현시점 시계열 정렬**(score 있는 것만).

    train/OOS 분할이 정직한 시간순이 되도록 정렬 — store 삽입순/백필 순서에 의존하지 않음.
    triggered_at(ISO) 없으면 registered_at, 둘 다 없으면 맨 뒤("~" > 숫자)로 밀어 원순서 보존.
    """
    tagged = []
    try:
        import store
    ...
    for r in rows:
        ...
        if sc is None or rm is None:
            continue
        try:
            ts = str(r.get("triggered_at") or r.get("registered_at") or "~")
            tagged.append((ts, float(sc), float(rm)))
        except (TypeError, ValueError):
            continue
    try:
        from ml import entry_feedback
        for r in entry_feedback.training_rows(horizon=20):
            sc, rm = r.get("score"), r.get("r_multiple")
            if sc is None or rm is None:
                continue
            ts = str(r.get("exit_date") or r.get("matured_at") or r.get("date") or "~")
            tagged.append((ts, float(sc), float(rm)))
    except Exception as e:
        logger.warning("entry_signals outcome 조회 실패: %s", e)
    tagged.sort(key=lambda x: x[0])      # ISO 문자열 사전순 = 시간순
    return [(sc, rm) for _, sc, rm in tagged]
...
def _best_threshold(samples: list[tuple[float, float]]) -> float:
    ...
    동점(평균 R 동일) 시 **더 보수적인(높은) 임계값** 선호 — THRESH_GRID 오름차순 + `>=` 로
    뒤쪽(높은) thr 이 동점을 덮어쓴다(더 선별적 = 신호 적게, 결정적 타이브레이크).
    """
