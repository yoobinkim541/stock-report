...
두 가지를 한다:
  ...
  2) 당일 리포트를 월/주 계층 페이지에 누적 아카이빙(crons/notion_archive.py) —
     📚 리포트 아카이브 → 26/06 → 4주차 → 일별 요약 토글 + 풀 리포트 페이지.
     멱등 upsert, 대시보드와 독립(아카이브 실패해도 대시보드 동기화는 계속).
     대시보드 하단엔 아카이브 링크를 단다.
...
def _load_full_report(report_date: str) -> str:
    """아카이브 전용 풀 Markdown 리포트 로드. 대시보드 인라인 노출에는 쓰지 않는다."""
    if not report_date:
        return ""
    from pathlib import Path
    path = Path.home() / "reports" / f"investment-report-{report_date}.md"
    try:
    ...
    try:
        ...
        for rdate, rtext in recent:
            archive_root_id = notion_archive.archive_report(
                rdate,
                rtext,
                full_text=_load_full_report(rdate),
            )
