...
AUTO_CURATE_MIN_LENGTH = 20
AUTO_CURATE_MAX_LENGTH = 4000
AUTO_CURATE_MIN_SCORE = 8
...
def auto_curate_from_chat(
    ...
    *,
    ...
    """대화 내용을 위키 카드로 자동 정리한다.

    LLM 이 가능하면 JSON 계획을 먼저 받아서 페이지를 만들고, 실패하거나 비활성화면
    보수적 휴휴리스틱으로만 동작한다. 자동 정리는 항상 기존 위키와 유사도 비교 후
    update/create 를 결정한다.
    """
...
def _should_auto_curate(question: str, answer: str) -> bool:
    text = f"{question}\n{answer}".lower()
    if len(question) < AUTO_CURATE_MIN_LENGTH or len(answer) < AUTO_CURATE_MIN_LENGTH:
        return False
    signals = (
        "손실한도", "레버리지", "리밸런", "포트폴리오", "전략", "시나리오",
        "기억", "위키", "정리", "교훈", "실패", "성공", "검증", "규칙",
        "백테스트", "가설", "체크리스트", "주의", "판단", "추천",
    )
...
def _build_auto_curation_prompt(
    *,
    ...
    lines = [
        ...
        "목표: 대화에서 재사용 가능한 규칙, 결정, 주의점을 하나의 위키 카드로 정리한다.",
...
def _heuristic_curation_plan(
    ...
    *,
    ...
    status = "draft"
    if any(token in text for token in ("규칙", "손실한도", "레버리지", "전략", "교훈", "검증")):
        status = "reviewed"
    ...
    plan = {
        ...
        "confidence": 0.68 if status == "reviewed" else 0.54,
